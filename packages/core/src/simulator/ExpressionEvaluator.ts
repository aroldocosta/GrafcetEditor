...
export interface SimulationVariableContext {
  ...
  timers: Map<number, boolean>;          // T1, T2... (True se o timer finalizou)
  counters: Map<number, boolean>;        // C1, C2... (True se atingiu o preset)
}
...
export class ExpressionEvaluator {
   ...
   */
  public static evaluate(expression: string, context: SimulationVariableContext): boolean {
  ...
  private static evaluateAnalogComparisons(expr: string, analogs: Map<number, number>): string {
  ...
  private static evaluateBooleanExpression(expr: string, context: SimulationVariableContext): boolean {
  ...
  private static resolveOperand(token: string, context: SimulationVariableContext): boolean {
