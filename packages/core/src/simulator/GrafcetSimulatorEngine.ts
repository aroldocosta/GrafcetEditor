...
export interface TimerState {
  id: number;
  funct: number;       // 1: TON, 2: TOFF, 3: INTERMITENTE
  presetMs: number;
  offsetMs: number;
  elapsedMs: number;
  done: boolean;
...
export interface SimulationState {
  ...
  timers: Record<number, { id: number; funct: number; elapsedMs: number; presetMs: number; offsetMs: number; done: boolean; active: boolean }>;
  cycleCount: number;
  isRunning: boolean;
}
...
export class GrafcetSimulatorEngine {
  ...
  public reset(): void {
    ...
    // Executar 1 ciclo inicial de temporizadores e ações
    this.updateTimers(0);
  ...
  private updateTimers(dtMs: number): void {
    ...
    // Descobrir quais timers estão associados a etapas atualmente ativas
    ...
    // Atualizar cada timer conforme o seu modo (funct):
    // 1: TON (On-Delay)
    // 2: TOFF (Off-Delay)
    // 3: INTERMITENTE (Oscilador Cíclico)
    this.timers.forEach((timer, channel) => {
      const isInputActive = activeTimerChannels.has(channel);
      timer.active = isInputActive;

      switch (timer.funct) {
        case 1: // TON - Temporizador ao Trabalho (On-Delay)
        default:
          if (isInputActive) {
            if (!timer.done) {
            ...
            // Etapa inativa: reseta o temporizador
            timer.elapsedMs = 0;
            timer.done = false;
          }
          break;

        case 2: // TOFF - Temporizador ao Repouso (Off-Delay)
          if (isInputActive) {
            // Enquanto a etapa está ativa, a saída fica imediatamente ligada
            timer.done = true;
            timer.elapsedMs = 0;
          } else {
            // Quando a etapa desativa, conta o atraso de desligamento mantendo a saída ligada
            if (timer.done) {
              timer.elapsedMs += dtMs;
              if (timer.elapsedMs >= timer.presetMs) {
                timer.elapsedMs = timer.presetMs;
                timer.done = false; // Desliga a saída ao atingir o preset
              }
            } else {
              timer.elapsedMs = 0;
            }
        ...
        case 3: // INTERMITENTE - Oscilador Cíclico (Blink)
          if (isInputActive) {
            ...
            // Durante [0, tOn[, saída é true; em [tOn, totalCycle[, saída é false
