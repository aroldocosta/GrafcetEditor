export class GrafcetStructureValidator {
    ...
    static validate(ir) {
        const errors = [];
        const warnings = [];
        const stepsMap = new Map(ir.steps.map(s => [s.id, s]));
        ...
        const transitionsToStep = new Map();
        ...
        for (const div of divergences) {
            ...
            const branchTerminalSteps = new Map();
            ...
            for (let i = 0; i < div.toSteps.length; i++) {
                for (let j = i + 1; j < div.toSteps.length; j++) {
                    ...
                    for (const sId of stepsI) {
                        if (stepsJ.has(sId)) {
                            errors.push(`Cruzamento parcial detectado (IEC 60848 item 3.1): A Etapa ${sId} pertence simultaneamente aos ramais ${i + 1} e ${j + 1} da Divergência ${div.id} sem passar por uma convergência em "E".`);
                        }
                    }
                            ...
                            if (t.fromSteps.length === 1) {
                                for (const targetId of t.toSteps) {
                                    if (stepsJ.has(targetId) && !stepsI.has(targetId)) {
                                        errors.push(`Cruzamento parcial proibido (IEC 60848 item 3.1): Transição ${t.id} salta do Ramal ${i + 1} (Etapa ${sId}) para o Ramal ${j + 1} (Etapa ${targetId}) no meio do paralelismo.`);
                                    }
                                }
                            }
                        }
                            ...
                            if (t.fromSteps.length === 1) {
                                for (const targetId of t.toSteps) {
                                    if (stepsI.has(targetId) && !stepsJ.has(targetId)) {
                                        errors.push(`Cruzamento parcial proibido (IEC 60848 item 3.1): Transição ${t.id} salta do Ramal ${j + 1} (Etapa ${sId}) para o Ramal ${i + 1} (Etapa ${targetId}) no meio do paralelismo.`);
                                    }
                                }
                            }
                        }
            ...
            for (let branchIdx = 0; branchIdx < div.toSteps.length; branchIdx++) {
                ...
                for (const termId of terminals) {
                    ...
                    if (conv) {
                        ...
                        for (let otherIdx = 0; otherIdx < div.toSteps.length; otherIdx++) {
                            if (otherIdx === branchIdx)
                                continue;
                            const otherSteps = branchSteps.get(otherIdx);
                            const hasFromOther = conv.fromSteps.some(fs => otherSteps.has(fs));
                            if (!hasFromOther) {
                                errors.push(`Convergência incompleta (IEC 60848): A Convergência ${conv.id} sincroniza o Ramal ${branchIdx + 1}, mas não inclui etapas do Ramal ${otherIdx + 1} da Divergência ${div.id}.`);
                            }
                        }
                    }
                }
                if (!branchConverged && terminals.size > 0) {
                    ...
                    errors.push(`Ramal sem fechamento (IEC 60848): O Ramal ${branchIdx + 1} da Divergência ${div.id} (terminando em Etapa(s) [${terminalList}]) não fecha em uma convergência em "E".`);
                }
            }
            ...
            for (const conv of candidateConvergences) {
                if (div.receptivity && conv.receptivity) {
                    ...
                    if (normDiv === normConv && normDiv !== '1' && normDiv !== 'TRUE') {
                        warnings.push(`Aviso de Projeto GRAFCET: A transição de abertura (Divergência ${div.id}) e a transição de fechamento (Convergência ${conv.id}) compartilham a mesma receptividade "${div.receptivity}". Isso pode causar disparo prematuro da convergência por corrida de sinal.`);
                    }
                }
            }
        }
